...
class Agent_customerController extends Controller
{
    ...
    public function add()
    {
        ...
        if (Auth::guard('admin')->user()->role_name !== 'Admin') {
            return redirect()->route('agent_customer.index')->with('error', 'คุณไม่มีสิทธิ์ในการเพิ่มข้อมูล');
        }
        // ส่งตัวแปรไปยัง View
    ...
    public function edit($id)
    {
        // ตรวจสอบสิทธิ์
        if (Auth::guard('admin')->user()->role_name !== 'Admin') {
            return redirect()->route('agent_customer.index')->with('error', 'คุณไม่มีสิทธิ์ในการแก้ไขข้อมูล');
        }
